...
namespace Test
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            if (extensionLibroEnRev == l1.GetNumPaginas + l2.GetNumPaginas) { puntos += 3; }
            ...
            if (extensionMapaDistr == m1.GetSuperficie) { puntos += 3; }
            if (cantidadMapaDistr == 1) { puntos += 1; }
            if (resumenMapaDistr == m1.ToString()) { puntos += 1; }

            if (extensionMapaEnEsc == m2.GetSuperficie) { puntos += 3; }
            ...
            if (extensionMapaTerminado == m3.GetSuperficie + m4.GetSuperficie) { puntos += 3; }
